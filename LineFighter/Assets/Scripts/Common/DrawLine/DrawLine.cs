...
public class DrawLine : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(1) && !stopHolding)
        {
        ...
        if (Input.GetMouseButtonUp(1) && !stopHolding)
        {
    ...
    private void CreateLine(Vector2 mousePosition)
    {
        ...
        currentLine.transform.parent = GameObject.FindGameObjectWithTag(Fields.GameObjects.PlayerLines).transform;
        currentLine.tag = Fields.Tags.LineObject;

        lineColor = Color.white;
        line = AssetLibrary.MaterialAssets["LineMaterial_Base"];
